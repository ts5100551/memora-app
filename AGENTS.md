...
|-------------|--------|-----------------|------------------|
| Local dev   | —      | `memora` | `.env.local` |
| Vercel Production | `main` | `memora` | Vercel → Production |
| Vercel Preview | `develop` and others | `memora` | Vercel → Preview |

A single shared Supabase project (`memora`) is used across all environments — this is a personal single-user project, no staging/prod split needed. See `docs/DEVELOPMENT.md` for setup details. Push to `main` deploys Production; push to `develop` or feature branches deploys Preview.
